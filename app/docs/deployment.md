...
3. Vercel production builds run `pnpm migrate:deploy` before `pnpm build` through the checked-in `pnpm build:vercel` command. The deployment migration holds a renewable MongoDB lease across all pending migrations and index verification, so overlapping builds cannot migrate concurrently. A migration or index failure rejects the deployment; preview, development, and custom-environment builds only run `pnpm build`. Keep migrations backward-compatible because the database can be upgraded even if the subsequent application build fails. For non-Vercel releases, run `pnpm migrate:status`, `pnpm migrate`, `pnpm verify:indexes`, generated checks, tests, and build against the target environment in a controlled job.
